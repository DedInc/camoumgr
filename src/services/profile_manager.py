...
class ProfileManager:
    ...
    def export_profile(
        ...
        try:
            ...
            profile_data_dir = self._data_path(name)
            ...
            logger.info("Exported profile %s to %s", name, zip_path)
            return True, zip_path
        except Exception as e:
            logger.exception("Error exporting profile %s: %s", name, e)
            return False, str(e)
            ...
            with zipfile.ZipFile(zip_path, "r") as zipf:
                ...
                if data_files:
                    ...
                    for file in data_files:
                        ...
                        pathlib.Path(pathlib.Path(target_path).parent).mkdir(
                            exist_ok=True,
                            parents=True,
                        )
                        with (
                            zipf.open(file) as src,
                            pathlib.Path(target_path).open("wb") as dst,
                        ):
                            dst.write(src.read())

                self.save_profiles()
                logger.info("Imported profile: %s", name)
                return True, name
